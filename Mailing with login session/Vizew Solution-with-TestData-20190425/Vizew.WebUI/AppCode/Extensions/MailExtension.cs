...
namespace Vizew.WebUI
{
    public static partial class Extension
    {
        static public bool SendMail(this string toMails,string subject, string body)
        {
            ...
            MailMessage mail = new MailMessage(new MailAddress(ConfigurationManager.AppSettings.Get("fromMail"), "Vizew App"), new MailAddress(toMails.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)[0]))
            {
                Subject = subject,
                Body = body,
                IsBodyHtml = true
